          ...
          <div class="row d-flex align-items-md-stretch">
            ...
            <div class="col-lg-12 col-md-12 flex-lg-last flex-md-first align-self-baseline">
              ...
              <div class="card sales-report mt-10px">
                ...
                <?php echo form_open(current_url());?>						
                    ...
                    <fieldset>
                        ...
                        <ul class="<?php echo ($summary_data[$this->flexi_cart_admin->db_column('order_status', 'cancelled')] == 1) ? 'order_status_cancelled' : 'order_status_active'; ?>">
                            ...
                            <?php 
                                $ordno = $summary_data[$this->flexi_cart_admin->db_column('order_summary', 'order_number')];
                                $query = $this->db->query("select tracking_company,tracking_no from order_summary where ord_order_number = '$ordno'");
                                $result = $query->result_array();
                            ?>
                            <li>

                                <strong class="spacer_125">Select Tracking Company:</strong>

                                <select name="tracking_company" class="width_175" required>
                                    <option value="Sendle" <?php if($result){ if($result[0]["tracking_company"] == "Sendle"){ echo "selected"; } } ?>>Sendle</option>
                                    <option value="Couriers Please" <?php if($result){ if($result[0]["tracking_company"] == "Couriers Please"){ echo "selected"; } } ?>>Couriers Please</option>
                                     <option value="Transdirect" <?php if($result){ if($result[0]["tracking_company"] == "Transdirect"){ echo "selected"; } } ?>>Transdirect</option>
                                    <option value="Australia Post" <?php if($result){ if($result[0]["tracking_company"] == "Australia Post"){ echo "selected"; } } ?>>Australia Post</option>
                                </select>

                            </li>   
                            <li>

                                <strong class="spacer_125">Enter Tracking Number:</strong>

                                <input type="text" name="tracking_no" value="<?php if($result){ echo $result[0]['tracking_no']; } ?>" required>
                        ...
                        <input type="checkbox" name="send_email_to_user" value="1" checked>Send email to user? 
                        <input type="checkbox" name="send_invoice_email_to_user" class="ml-4" value="1">Send invoice email to user?
                        <hr>
                    ...
                    <fieldset>
                        ...
                        <div class="row">
                        <div class="col-md-4">
                            <strong class="spacer_125">Update Payment Status:</strong>
                            <select class="form-control" id="paymentchangestatus" onchange="paymentchange()">
                                <option value="" <?php if($summary_data['payment_status'] == ''){ echo 'selected';} ?>>Awaiting Payment</option>
                                <option value="Completed" <?php if($summary_data['payment_status'] == 'Completed'){ echo 'selected';} ?>>Payment Received</option>
                            </select>
                        </div>

                    </fieldset>
                    <fieldset>
                        <legend>Payment Log History</legend>
                        <button type="button" class="btn btn-all" data-toggle="modal" data-target="#exampleModal">
                            Add Payment Details
                        </button>
                        <table>
                            <thead>
                            ...
                            <tbody>
                                <?php 
                               $ono =  $summary_data[$this->flexi_cart_admin->db_column('order_summary', 'order_number')];
                                    $query = $this->db->query("select * from payment_log_history where orderno='$ono'");
                                    $resultdata = $query->result_array();
                                    if($resultdata){
                                        foreach($resultdata as $key){
                                ?>
                                <tr>
                                    <td><?php echo $key['dateofpayment']; ?></td>
                                    <td><?php echo $key['via']; ?></td>
                                    <td><?php echo "$".$key['amount']; ?></td>
                                    <td><?php echo $key['description']; ?></td>
                                    <td><a href="<?php echo base_url().ADMIN_FOLDER; ?>/deletepaymentoforder/<?php echo $key['orderno']; ?>" class="btn btn-all">Delete</a></td>
                                </tr>
                                <?php } } else { ?>
                                <tr>
                                    <td colspan="5">
                                        <center>
                                        There Is Not Log Yet!
                                        </center>
                                    </td>
                                </tr>
                                <?php } ?>
                    ...
                    <fieldset>
                        <legend>Order Log History</legend>
                        <table>
                            <thead>
                                <tr>
                                    <th>Date Of Log</th>
                            ...
                            <tbody>
                                <?php 
                               $ono =  $summary_data[$this->flexi_cart_admin->db_column('order_summary', 'order_number')];
                                    $query = $this->db->query("select * from order_log_history where orderno='$ono'");
                                    $resultdata = $query->result_array();
                                    if($resultdata){
                                        foreach($resultdata as $key){
                                ?>
                                <tr>
                                    <td><?php echo $key['dateoflog']; ?></td>
                                    <td><?php $statusid = $key['orderstatus'];
                                          $query = $this->db->query("select ord_status_description from order_status where ord_status_id='$statusid'");  
                                          $statusdata = $query->result_array();
                                          echo $statusdata[0]['ord_status_description'];
                                    ?></td>
                ...
                <td><?php echo  $this->flexi_cart_admin->format_currency($summary_data['total_paid']/11); ?></td>
            <?php } else { ?>
          ...
          <?php }?>
                                <tr class="grand_total">
                                    ...
                                    <td>
                                                    <?php
                if( $summary_data['payment_type'] == 2){
                 echo $this->flexi_cart_admin->format_currency($summary_data['total_paid']);
             }else{
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        ...
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Add Payment Details Form</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
            </button>
        </div>
        <form action="<?php echo base_url().ADMIN_FOLDER; ?>/addpaymentoforder/<?php echo $summary_data[$this->flexi_cart_admin->db_column('order_summary', 'order_number')]; ?>" method="post">
            <div class="modal-body">
                    <div class="row order_status_active">
                        <div class="col-md-4">
                            <strong class="spacer_125">Payment Date:</strong>
                            <input type="date" name="paymentdate" class="form-control" value="" required="">
                        </div>
                        <div class="col-md-4">
                            <strong class="spacer_125">Payment Via:</strong>
                            <input type="text" name="paymentvia" class="form-control" value="" required="">
                        </div>   
                        <div class="col-md-4">
                            <strong class="spacer_125">Amount:</strong>
                            <input type="number" name="amount" class="form-control" value="" required="">
                        </div>   
                        <div class="col-md-12">
                            <strong class="spacer_125">Description:</strong>
...
<script>
    function paymentchange(){
        var status = $('#paymentchangestatus').val();
        $.ajax({
              method: "POST",
              url: "<?php echo base_url().ADMIN_FOLDER; ?>/paymentoforderstatus/<?php echo $summary_data[$this->flexi_cart_admin->db_column('order_summary', 'order_number')]; ?>",
              data: { status: status }
        }).done(function( msg ) {
