          ...
          <div class="row d-flex align-items-md-stretch">
            ...
            <div class="col-lg-12 col-md-12 flex-lg-last flex-md-first align-self-baseline">
              <div class="card sales-report mt-10px">
                <h2 class="display h4">All News <a href="<?php echo base_url().ADMIN_FOLDER."/news/add";?>" class="btn btn-success float-right" style="color:#fff;font-size: 13px;padding: 5px 10px;">Add News</a></h2>
                <p>A list of all News</p>
                <div class="table-responsive">
                    <table id="datatable" class="display responsive nowrap align-middle text-truncate mb-0 table table-bordered table-hover table-striped" style="width:100%;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                  <div class="checkbox-container"><label class="checkbox-label"><input type="checkbox" form="myform" name="check[]" id="<?php echo $news->id;?>" value="<?php echo $news->id;?>"><span class="checkbox-custom rectangular"></span></label></div>
                                    <?php echo $news->id;?>
                                </td>
                                <td>
                                    <?php echo $news->title;?>
                                ...
                                <td>
                                    <?php
                                        echo getnewsCategories($news->id);
                                    ?>
                                </td>
                                <td><?php if($news->is_active==1){?><label class="badge badge-success">Yes</label><? }else{?><label class="badge badge-danger">No</label><? }?></td>
                                <td><input type="number" name="<?php echo $news->id;?>"  form="myform" value="<?php echo $news->sort;?>" style="max-width:50px;"></td>
                                <td><?php echo timesincenew($news->created)." ago";?></td>
                                <td>
                                
                                <a title="Copy" class="btn btn-success" href="<?php echo base_url().ADMIN_FOLDER.'/news/copy-data/'.$news->id;?>"><i class="fa fa-copy f-16"></i></a>
                                <a title="Edit" class="btn btn-info" href="<?php echo base_url().ADMIN_FOLDER.'/news/edit/'.$news->id;?>"><i class="fa fa-pencil-square-o f-16"></i></a>
                                <a title="Delete" class="btn btn-danger" href="<?php echo base_url().ADMIN_FOLDER.'/news/delete/'.$news->id;?>" onclick="return confirm('Are you sure you want to delete this record?');"><i class="fa fa-trash f-16"></i></a>
                    ...
                    echo form_open(ADMIN_FOLDER.'/news/multi-data',$attributes2);
                    ?>
