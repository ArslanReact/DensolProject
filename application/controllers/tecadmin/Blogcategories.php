...
class BlogCategories extends MY_Controller {
    ...
    public function index($id=0)
    {
        ...
        $this->get_blogcategories($id);
    }
    public function copy_data($id)
    {
        $parent_id = $this->blogcategoriesModel->getValue("parent_id","id",$id);
        $this->blogcategoriesModel->DuplicateMySQLRecord($id);
    ...
    public function edit($id=null)
    {
        if($id){
            ...
            $data['page_heading'] = "Blog Categories";
            $data['page_parent'] = $parent_id;
            $data['blogcategoriescategories'] = $this->get_categories($parent_id,$id);
            $data['blogcategory'] = $this->blogcategoriesModel->get_single_category($id);
            $this->load->view(ADMIN_VIEW_FOLDER.'/blog/categories/edit',$data);
        }else{
            redirect(ADMIN_FOLDER.'/blog/categories');
        }
    }
    public function update()
    {
        ...
        $blogcategory = $this->blogcategoriesModel->get_single_category($pp);
        ...
        else
        {
            ...
            if(isset($del_icon)){
                ...
                $this->blogcategoriesModel->update_blogcategory($pp,$data,false,false);
                $delicon = null;
                delFile('blogs_categories','icon','/blog/categories/',$blogcategory->icon);
            }else{
                $delicon = $blogcategory->icon;
            }
            if(isset($del_thumbnail)){
                $data = array("thumbnail" => '');
                $this->blogcategoriesModel->update_blogcategory($pp,$data,false,false);
                $delthumbnail = null;
                delFile('blogs_categories','thumbnail','/blog/categories/',$blogcategory->thumbnail);
            }else{
                $delthumbnail = $blogcategory->thumbnail;
            }
            if(isset($del_banner)){
                $data = array("banner" => '');
                $this->blogcategoriesModel->update_blogcategory($pp,$data,false,false);
                $delbanner = null;
                delFile('blogs_categories','banner','/blog/categories/',$blogcategory->banner);
            }else{
                $delbanner = $blogcategory->banner;
            }
            ...
            $config['upload_path']   = UPLOADPATH.'blog/categories/';
            ...
            if($this->blogcategoriesModel->update_blogcategory($pp,$postdata,$updateslug)){
                if($ddii==1){delFile('blogs_categories','icon','/blog/categories/',$delicon);}
                if($ddtt==1){delFile('blogs_categories','thumbnail','/blog/categories/',$delthumbnail);}
                if($ddbb==1){delFile('blogs_categories','banner','/blog/categories/',$delbanner);}
                $this->session->set_flashdata('etype', 'alert-success');
                $this->session->set_flashdata('emsg', 'Blog category successfully updated');
    ...
    public function add($id=0)
    {
        ...
        $this->load->view(ADMIN_VIEW_FOLDER.'/blog/categories/add',$data);
    }
    public function delete($id)
    {
        $blogcategory = $this->blogcategoriesModel->get_single_category($id);
        if(isset($blogcategory)){
            if($this->blogcategoriesModel->is_parent($id)==1){
                $this->session->set_flashdata('etype', 'alert-error');
                $this->session->set_flashdata('emsg', 'please delete child categories first');
                $this->cms_redirect(0,$blogcategory->parent_id,'/');
            }else{
                $blogcategoryicon = $blogcategory->icon;
                $blogcategorythumb = $blogcategory->thumbnail;
                $blogcategorybanner = $blogcategory->banner;
                if($this->blogcategoriesModel->delete_category($id)){
                    delFile('blogs_categories','icon','/blog/categories/',$blogcategoryicon);
                    delFile('blogs_categories','thumbnail','/blog/categories/',$blogcategorythumb);
                    delFile('blogs_categories','banner','/blog/categories/',$blogcategorybanner);
                    $this->blogcategoriesModel->update_sel_categories($id);
                    $this->session->set_flashdata('etype', 'alert-success');
                    $this->session->set_flashdata('emsg', 'blog category successfully deleted');
                    $this->cms_redirect(0,$blogcategory->parent_id,'/');
                }else{
                    $this->session->set_flashdata('etype', 'alert-error');
            ...
            if($sort_multi){
                foreach($checks as $check){
                    ...
                    if($sort != ""){
                        ...
                        $this->blogcategoriesModel->update_blogcategory($check,$data,false,false);
                        array_push($succ,$this->blogcategoriesModel->getValue("title","id",$check)." sort Updated");
                    }else{
                        array_push($erro,$this->blogcategoriesModel->getValue("title","id",$check)." sort is not defined!");
                    }
                }
            }
            if($del_multi){
                foreach($checks as $check){
                    if($this->blogcategoriesModel->is_parent($check)==1){
                        array_push($erro,"(".$this->blogcategoriesModel->getValue("title","id",$check).") have child pages");
                    }else{
                        $blogcategory = $this->blogcategoriesModel->get_single_category($check);
                        if(isset($blogcategory)){
                            $blogcategoryicon = $blogcategory->icon;
                            $blogcategorythumb = $blogcategory->thumbnail;
                            $blogcategorybanner = $blogcategory->banner;
                            array_push($succ,$this->blogcategoriesModel->getValue("title","id",$check)." Deleted successfully");
                            $this->blogcategoriesModel->delete_category($check);
                            delFile('blogs_categories','icon','/blog/categories/',$blogcategoryicon);
                            delFile('blogs_categories','thumbnail','/blog/categories/',$blogcategorythumb);
                            delFile('blogs_categories','banner','/blog/categories/',$blogcategorybanner);
                            $this->blogcategoriesModel->update_sel_categories($check);
                        }
                    }
                }
            }
    ...
    {
        $categories = $this->blogcategoriesModel->get_all_blogcategories(0,true);
    ...
    public function add_submit($id=null)
    {
        ...
        else
        {
            ...
            if($this->blogcategoriesModel->add_blogcategory($slug,$postdata)){
                $this->session->set_flashdata('etype', 'alert-success');
    ...
    public function get_blogcategories($id)
    {
        $data['sessions'] = $this->session->userdata();
        $data['page_heading'] = "Blog Categories";
        $data['page_parent'] = $id;
        $data['blogcategories'] = $this->blogcategoriesModel->getblogcategories($id);
        $this->load->view(ADMIN_VIEW_FOLDER.'/blog/categories/index',$data);
    }
    public function cms_redirect($p1,$p2,$path)
    {
        if($p1==0){
            if($p2 != 0){
                redirect(ADMIN_FOLDER.'/blog/categories'.$path.$p2);
            }else{
                redirect(ADMIN_FOLDER.'/blog/categories'.$path);
            }
        }else{
            redirect(ADMIN_FOLDER.'/blog/categories'.$path.$p1);
        }
    }
}