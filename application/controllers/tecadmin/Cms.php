...
class Cms extends MY_Controller {
    ...
    public function index($id=0)
    {
     ...
     public function testimonial($id=0)
    {
        //echo date("Y-m-d h:i:s");
        //var_dump($this->session->flashdata());
        $this->get_testi($id);
    ...
    public function delete_testi($id)
    {
        ...
        $this->db->where('id', $id);
             //$this->cms_redirect(0,$cms->parent_id,'/');   
             redirect('https://densol.com.au/admin/testimonial');
     ...
     public function get_testi($id)
    {
        ...
        //$data['cmss'] = $this->db('select * from testimonial');
        
        $this->load->view(ADMIN_VIEW_FOLDER.'/cms/testimonial',$data);
    }
     public function update_testi($id)
    {
        $data['sessions'] = $this->session->userdata();
        $data['page_heading'] = "Update Testimonials";
        $query = $this->db->query("select * from testimonial where id='$id'");
        $data['result'] = $query->result_array();
        $this->load->view(ADMIN_VIEW_FOLDER.'/cms/update_test',$data);
    }    
    public function test_update_db(){
		...
		if($this->upload->do_upload('image'))
		{
			...
			$picaddress = 'uploads/testimonial/'.$data['upload_data']['file_name'];
		}
		if($picaddress == ""){
    		$array = array(
    		    "title" => $title,
        ...
        $this->session->set_flashdata("info", "Testimonial Comment Successfully Update.");
        $this->get_testi($id);
    }
