...
class Products extends MY_Controller {
    ...
    public function export_product()
    {
	   ...
	   // get data 
// 		$productsData = $this->db->query("SELECT products.*,products_categories.title as cname FROM `products` 
//                                         left outer join products_sel_categories on products_sel_categories.product_id = products.id and products_sel_categories.productcategory_id in (18,26)
//                                         left outer join products_categories on products_categories.id = products_sel_categories.productcategory_id");
        $productsData = $this->db->query("SELECT products.*,products_categories.title as cname FROM `products_sel_categories` left outer join products on products.id = products_sel_categories.product_id left outer join products_categories on products_categories.id = products_sel_categories.productcategory_id where productcategory_id not in (18,26) order by productcategory_id ASC");                                
		$productsData = $productsData->result_array();
		// file creation 
		$file = fopen('php://output','w');
		$header = array("Product ID","Title","Category Name","SKU","Short Description","Full Description","Image Link","Price","Sale Price","Tags","Meta Title","Meta Description", "Meta Keywords"); 
		fputcsv($file, $header);
		foreach ($productsData as $key){ 
		    
		    $imglink = base_url()."uploads/products/".$key['image'];
		    $pid = $key['id'];
		    $tagss = '';
		    $query = $this->db->query("select * from tags where t_table='products' and t_table_id='$pid'");
		    $results = $query->result_array();
		    if($results){foreach($results as $data){
		        $tagss .= $data['normal_tag'].',';
		    }
		        $tagss = rtrim($tagss, ',');
		    }
		    
		    //
		    $stockData = $this->db->query("SELECT groups_prices.price,groups_prices.sale_price FROM item_stock inner join products on products.id = item_stock.stock_item_fk inner join groups_prices on groups_prices.pid = item_stock.stock_item_fk where products.id='$pid'");
		    $stockData = $stockData->result_array();
		    $price = $stockData[0]['price'];
		    $slaeprice = $stockData[0]['sale_price'];
		    
		    $lists = array(
                        "id" => $key['id'],
                        "title" => $key['title'],
                        "category_name" => $key['cname'],
                        "article" => $key['article'],
                        "short_detail" => $key['short_detail'],
                        "full_detail" => $key['full_detail'],
                        "image" => $imglink,
                        "price" => $price,
                        "sale_price" => $slaeprice,
                         "tagss" => $tagss,
                        "meta_title" => $key['meta_title'],
                        "meta_desc" => $key['meta_desc'],
    ...
    public function export_fbproduct()
    {
		// file name 
		$filename = 'products_'.date('Ymd').'.csv'; 
		header("Content-Description: File Transfer"); 
...
//                                         left outer join products_categories on products_categories.id = products_sel_categories.productcategory_id");
		...
		// file creation 
		$file = fopen('php://output','w');
		$header = array("id","override","price","link","image_link","sale_price","title","Description"); 
		fputcsv($file, $header);
		foreach ($productsData as $key){ 
		    
		    $imglink = base_url()."uploads/products/".$key['image'];
		    $link = base_url()."product/".$key['slug'];
		    $pid = $key['id'];
		    $tagss = '';
		    $query = $this->db->query("select * from tags where t_table='products' and t_table_id='$pid'");
		    $results = $query->result_array();
		    ...
		    $price = $stockData[0]['price'];
		    $slaeprice = $stockData[0]['sale_price'];
		    $str = strip_tags(cleanout($key['short_detail']));
    ...
    public function export_stock()
    {
		...
		header("Content-Type: application/csv; ");
	   // get data 
		$stockData = $this->db->query("SELECT item_stock.stock_item_fk, products.article, item_stock.stock_quantity,groups_prices.price,groups_prices.sale_price FROM item_stock inner join products on products.id = item_stock.stock_item_fk inner join groups_prices on groups_prices.pid = item_stock.stock_item_fk");
		$stockData = $stockData->result_array();
		// file creation 
		$file = fopen('php://output','w');
		$header = array("Product ID","SKU","Stock Qty","Price","Sale Price"); 
		fputcsv($file, $header);
		foreach ($stockData as $key=>$list){ 
        fputcsv($file,$list); 
		}
		fclose($file); 
		exit; 
      ...
      public function import_details(){
       $data['sessions'] = $this->session->userdata();
        $data['page_heading'] = "Import Products";
        $this->load->view(ADMIN_VIEW_FOLDER.'/products/import_product_detail',$data); 
    }

    public function import(){
        $data['sessions'] = $this->session->userdata();
        $data['page_heading'] = "Import Products";
        $this->load->view(ADMIN_VIEW_FOLDER.'/products/import',$data);       
    }
    public function add_importindb()
    {
		...
		if(isset($_FILES["getFile"]["name"]))
		{
			...
			foreach($object->getWorksheetIterator() as $worksheet)
			{
				...
				for($row=2; $row<=$highestRow; $row++)
				{
					...
					$sprice = $worksheet->getCellByColumnAndRow(4, $row)->getValue();
					$data = array(
						'stock_quantity'	=>	$stock
					);
					$this->db->where('stock_item_fk', $pid);
					...
					$this->db->update('groups_prices', $pricedata);					
				}
			}
			
                $this->session->set_flashdata('etype', 'alert-success');
                $this->session->set_flashdata('emsg', 'Product Stock Successfully Updated');
                $this->cms_redirect($pp,0,'/');
		}	      

    }
    public function add_productdetails_importindb(){
          $this->load->library('excel');
		if(isset($_FILES["getFile"]["name"]))
		{
			$path = $_FILES["getFile"]["tmp_name"];
			...
			foreach($object->getWorksheetIterator() as $worksheet)
			{
				...
				for($row=1; $row<=$highestRow; $row++)
				{
					...
					$article = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
					$is_featured = $worksheet->getCellByColumnAndRow(3, $row)->getValue();
					$promotion = $worksheet->getCellByColumnAndRow(4, $row)->getValue();
					$is_active = $worksheet->getCellByColumnAndRow(5, $row)->getValue();
                ...
                $this->session->set_flashdata('etype', 'alert-success');
                $this->session->set_flashdata('emsg', 'Product Detail Successfully Updated');
                $this->cms_redirect($pp,0,'/');
		}
    ...
    public function update()
    {
      ...
      //  $this->form_validation->set_rules('img_alt', 'Img_alt', 'trim|required|max_length[250]');
        ...
        else
        {
            ...
            $article = doSeo(sanitize($this->input->post('article',TRUE)));
            $short_detail = sanitize($this->input->post('short_detail',TRUE));
            $full_detail = sanitize($this->input->post('full_detail',FALSE));
