...
class Demo_cart_model extends CI_Model {
	...
	function demo_insert_form_item_to_cart($item_id = 0)
	{
		...
		{
			$item_id = $this->input->post('id');
			$product = getSingleValuee("products","id",$item_id);
			$productprcs = getuserprdprice($item_id);
			$submit = $this->input->post('offer');
			
			if (isset($_POST['submit'])) {
			   $quantity = $this->input->post('qty2');
			   $price = $this->input->post('qty_price');
			...
			if($uid != ''){
				...
				$disc_amount = $scms->discount;
			}

			$price = $productprcs['new_price'];
			
			$product = $disc_amount / 100 * $price;
			$final = $price-$product;
			$price = $final;
			...
			if($options){
				...
				foreach($options as $option){
					$dc = explode(",",$option);
					if($dc[0] != 0){
						$variation = getSingleValuee("variations","id",$dc[0]);
						if($variation->price_option=="+"){
	...
	function demo_save_order()
	{
		...
		if ($this->form_validation->run())
		{
			...
			$userdataIp= $this->input->ip_address();
			$googlesecret = "6LeGRYEaAAAAAPiveMhAimMkav6TeKhj5ewcsOho";
			$googleverifyurl="https://www.google.com/recaptcha/api/siteverify?secret=".$googlesecret."&response=".$captcharesponse."&remoteip=".$userdataIp;
			$ch = curl_init(); 
			curl_setopt($ch, CURLOPT_URL, $googleverifyurl); 
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); 
			$googleoutput = curl_exec($ch); 
			curl_close($ch);

			$googlestatus= json_decode($googleoutput, true);
			// if (!$googlestatus['success']) {
			// 	$this->data['message'] = '<p class="error_msg">Invalid Captcha</p>';
			// 	return FALSE;
			// }
			// if($this->session->userdata('captcha') != $order_data['captcha']){
			// 	$this->data['message'] = '<p class="error_msg">Invalid Captcha('.$this->session->userdata('captcha').'='.$order_data['captcha'].')</p>';
			// 	return FALSE;
			// }
