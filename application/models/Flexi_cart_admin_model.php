...

{
...

		{
...

			{
...

				{
					...
					$order_number = $prefix.'00035015'.$suffix;
...

	{
		...
		$user_id = (isset($custom_summary_data[$this->flexi->cart_database['order_summary']['columns']['user']])) ? $custom_summary_data[$this->flexi->cart_database['order_summary']['columns']['user']] : 0;
	 ...
	 */
		...
		public function email_order($order_number = FALSE, $email_to = FALSE, $email_subject = FALSE, $custom_data = FALSE, $pdffile = FALSE)

	{
...

		{
			...
			// $sql_where = array($this->flexi->cart_database['order_summary']['columns']['user'] => 1);
...

			{
...

				{
					...
					$ss = site_settings();
					$email_subject = $mail->Subject = 'Order Placed '.$ss->company_name;
				...
				// 	// $mail->Subject = (empty($email_subject)) ? $ss->company_name.' : Order Details' : $email_subject;
				// 	$mail->Subject = 'Order Placed '.$ss->company_name;
					
				// 	$mail->isHTML(true);
				// 	$message = $this->load->view($email_settings['email_template'], $order_data, TRUE);
				// 	$mail->Body = $message;
				// 	if($pdffile){
				// 		$mail->AddAttachment($pdffile, ORDER_NUMBER_PREFIX.$order_number.".pdf",  'base64', 'application/pdf');
				// 	}
				// 	if(!$mail->send()){
				// 	log_message('Debug', 'Mailer Error: '. $mail->ErrorInfo);
				// 	return false;
				// 	}else{
				// 	log_message('Debug', 'Email Sent Success to ('.$email_to.'&'.$ss->company_email.')');
				// 	return true;
				// 	}
				// 	if($pdffile){
				// 		unlink($pdffile);
