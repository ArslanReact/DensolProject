...
class NewsCategoriesModel extends CI_Model {
    ...
    public function getnewscategories($id=0)
    {
    ...
    public function get_all_newscategories($id=0,$category=false)
    {
        if(!$category){
    ...
    public function update_newscategory($id,$data,$slug=false,$update=true)
    {
        if($slug != false){
    ...
    public function add_newscategory($slug,$data=array())
    {
        $slugarr = array('slug' => $this->create_slug($slug));
    ...
    {
        $this->db->where("newscategory_id", $id);
        $query = $this->db->get($this->selcattable);
        if ($query->num_rows() > 0){
            $data = array('newscategory_id' => 0);
            $this->db->update($this->selcattable, $data, "newscategory_id = ".$id);
            return true;
        }else{
            return false;
        }
