...
class Demo_cart_admin_model extends CI_Model {
	...
	function demo_resave_order($order_number)
	{
		...
		if (isset($this->post_data['save']))
		{
			...
			redirect(ADMIN_FOLDER.'/order_details/'.$order_number);
		}
		else
		{
	...
	function demo_insert_item_to_cart()
	{
		...
		foreach($this->input->post('insert_item') as $item_id => $item_post_data)
		{
			...
			{
				$query = $this->db->select('title, weight')
					->from('products')
					->where('id', $item_id)
					->get();
	...
	{	
		return $this->db->from('products')
			->join($this->flexi_cart_admin->db_table('item_stock'), 'id = '.$this->flexi_cart_admin->db_column('item_stock', 'item'))
			// ->join('products_sel_categories', 'product_id = id')
			->order_by('id')
			->get()
			->result_array();
	}
	...
	function demo_insert_location_type()
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			// Set a message to the CI flashdata so that it is available after the page redirect.
			$this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
			redirect(ADMIN_FOLDER.'/location_types');
		}
		else
		{
	...
	function demo_insert_location($location_type_id)
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			// Set a message to the CI flashdata so that it is available after the page redirect.
			$this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
			redirect(ADMIN_FOLDER.'/locations/'.$location_type_id);
		}
		else
		{
			// Set validation errors and field name ids so that data can be repopulated for all rows.
	...
	function demo_insert_shipping()
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			redirect(ADMIN_FOLDER.'/shipping');
		}
		else
		{
			// Set validation errors and field name ids so that data can be repopulated for all rows.
	...
	function demo_insert_shipping_rate($shipping_id)
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			// Set a message to the CI flashdata so that it is available after the page redirect.
			$this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
			redirect(ADMIN_FOLDER.'/shipping_rates/'.$shipping_id);
		}
		else
		{
			// Set validation errors and field name ids so that data can be repopulated for all rows.
	...
	function demo_insert_item_shipping($item_id)
	{
		...
		// Set a message to the CI flashdata so that it is available after the page redirect.
		$this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
		redirect(ADMIN_FOLDER.'/item_shipping/'.$item_id);
	}
	...
	function demo_insert_item_tax($item_id)
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			// Set a message to the CI flashdata so that it is available after the page redirect.
			$this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
			redirect(ADMIN_FOLDER.'/item_tax/'.$item_id);
	...
	function demo_update_order_details($order_number)
	{
		...
		$tcompany = $this->input->post('tracking_company');
		$tno = $this->input->post('tracking_no');
        $this->db->query("update order_summary set tracking_company='$tcompany', tracking_no='$tno' where ord_order_number='$order_number'");
	...
	function demo_insert_order_status()
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			$this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
			redirect(ADMIN_FOLDER.'/order_status');
		}
		else
		{
			// Set validation errors and field name ids so that data can be repopulated for all rows.
	...
	function demo_insert_currency()
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			$this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
			redirect(ADMIN_FOLDER.'/currency');
		}
		else
		{
			// Set validation errors and field name ids so that data can be repopulated for all rows.
	...
	function demo_update_discount($discount_id)
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			if ($this->flexi_cart_admin->update_db_discount($sql_update, $discount_id))
			{
				...
				$this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
				redirect(ADMIN_FOLDER.'/'.$redirect_page);
			}
			else
			{
				// Set errors.
	...
	function demo_insert_discount()
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			$this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
			redirect(base_url().ADMIN_FOLDER.'/'.$redirect_page);
		}
		else
		{
			// Set validation errors.
	...
	function demo_insert_discount_group_old()
	{
		$this->load->library('form_validation');
		...
		if ($this->form_validation->run()) 
		{
			...
			$query = $this->db->select('id')->from('products')->get();
			...
			if ($query->num_rows() > 0)
			{
				...
				if ($group_id)
				{
					foreach($query->result_array() as $item)
					{
						$sql_insert = array(
							...
							$this->flexi_cart_admin->db_column('discount_group_items', 'item') => $item['id']
						);
						echo "<pre>";
						print_r($sql_insert);
						echo "</pre>";
				...
				die();
				$this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
				redirect(ADMIN_FOLDER.'/update_discount_group/'.$group_id);
			}
			else
			{
				$this->data['message'] = '<div class="alert alert-error">There are no items matching the submitted SQL WHERE statement.</div>';
				$this->data['validation_row_ids'] = $row_ids;
		...
		if ($this->form_validation->run()) 
		{
			...
			if ($query->num_rows() > 0)
			{
				...
				foreach($query->result_array() as $item_data)
				{
					$sql_insert = array(
						$this->flexi_cart_admin->db_column('discount_group_items', 'group') => $group_id,
						$this->flexi_cart_admin->db_column('discount_group_items', 'item') => $item_data['id']
					);
