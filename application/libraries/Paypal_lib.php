...
class Paypal_lib {
...

	{
        ...
        $dbPayments = $query->row();

		if($dbPayments->is_live==1){
			$sanbox = FALSE;
		}else{
			$sanbox = TRUE;
		}
		...
		if($dbPayments->is_live==1){
			$businessEmail = $dbPayments->email;
		}else{
			$businessEmail = "usmanawaanforpaypal@gmail.com";
		}

		$this->add_fieldd('business',$businessEmail);

		$this->add_fieldd('rm','2');			  // Return method = POST

		$this->add_fieldd('cmd','_xclick');

		$this->add_fieldd('currency_code', $this->CI->config->item('paypal_lib_currency_code'));

	    // $this->add_fieldd('quantity', '1');
	...
	function add_fieldd($field, $value) 
...

	{
        ...
        echo '<p style="text-alight:center"><img src="'.base_url('files/frontend/images/loader.gif').'" style="margin:0 auto;"></p>';

        echo '<p style="text-align:center;font-family:Arial;">Please wait, your order is being processed and you will be redirected to the paypal website.</p>' . "\n";
